...
//!
//! These tests intentionally compare the current `lunar-lite` API against
//! `tyme4rs` as an external reference. A failure here is useful signal for the
//! migration decision: it means the behavior is not a drop-in match and needs an
//! adapter, a policy switch, or a documented incompatibility.

use lunar_lite::{
    EarthlyBranch, FourPillars, HeavenlyStem, LunarDate, SolarDate, StemBranch,
    StemBranchOptions, lunar_month_days, lunar_to_solar, lunar_year_branch, lunar_year_stem,
    lunar_year_stem_branch, normalize_lunar_date, solar_to_lunar, time_index,
    time_index_to_branch, validate_lunar_date,
};
use tyme4rs::tyme::lunar::{
    LunarDay as TymeLunarDay, LunarMonth as TymeLunarMonth, LunarYear as TymeLunarYear,
};
use tyme4rs::tyme::sixtycycle::SixtyCycle as TymeSixtyCycle;
use tyme4rs::tyme::solar::{SolarDay as TymeSolarDay, SolarTime as TymeSolarTime};
...
fn tyme_solar_day(date: SolarDate) -> TymeSolarDay {
...
fn lunar_from_tyme(day: TymeLunarDay) -> LunarDate {
...
fn solar_from_tyme(day: TymeSolarDay) -> SolarDate {
    SolarDate {
        year: day.get_year() as i32,
...
fn tyme_lunar_from_solar(date: SolarDate) -> LunarDate {
    lunar_from_tyme(tyme_solar_day(date).get_lunar_day())
}

fn tyme_solar_from_lunar(date: LunarDate) -> SolarDate {
...
fn stem_from_index(index: usize) -> HeavenlyStem {
    HeavenlyStem::from_index(index)
}

fn branch_from_index(index: usize) -> EarthlyBranch {
    EarthlyBranch::from_index(index)
}

fn stem_branch_from_tyme(value: TymeSixtyCycle) -> StemBranch {
    StemBranch::try_new(
        stem_from_index(value.get_heaven_stem().get_index()),
        branch_from_index(value.get_earth_branch().get_index()),
    )
    .expect("tyme4rs SixtyCycle should always be a valid stem-branch pair")
}

fn tyme_four_pillars(date: SolarDate, time_idx: u8) -> FourPillars {
...
fn solar_to_lunar_matches_tyme4rs_for_sampled_supported_dates() {
    ...
    for date in dates {
        assert_eq!(solar_to_lunar(date).unwrap(), tyme_lunar_from_solar(date));
    }
}
...
fn lunar_to_solar_matches_tyme4rs_for_representative_dates() {
    ...
    for date in dates {
        assert_eq!(lunar_to_solar(date).unwrap(), tyme_solar_from_lunar(date));
    }
}
...
fn strict_lunar_validation_matches_tyme4rs_validity_for_representative_dates() {
    ...
    for date in valid_dates {
        assert_eq!(validate_lunar_date(date), Ok(()));
        assert_eq!(lunar_to_solar(date).unwrap(), tyme_solar_from_lunar(date));
    }

    // `lunar-lite` deliberately normalizes fake leap flags for conversion.
...
fn lunar_year_stem_branch_helpers_match_tyme4rs() {
    for year in [1850, 1900, 1984, 2000, 2024, 2033, 2100, 2150] {
        ...
        assert_eq!(lunar_year_stem_branch(year), expected);
        assert_eq!(lunar_year_stem(year), expected.stem());
        assert_eq!(lunar_year_branch(year), expected.branch());
    }
}
...
fn time_index_semantics_match_tyme4rs_lunar_hour_index() {
    for (hour, expected_index) in [
        ...
        assert_eq!(time_index(hour, 30).unwrap(), expected_index);
...
fn default_exact_four_pillars_match_tyme4rs_for_sampled_dates_and_time_indices() {
    ...
    for (date, time_idx) in cases {
        let lunar_lite = lunar_lite::four_pillars_from_solar_date(date, time_idx).unwrap();
...
fn explicit_default_options_match_alias_and_tyme4rs() {
    ...
    let time_idx = 5;
    let alias = lunar_lite::four_pillars_from_solar_date(date, time_idx).unwrap();
    let explicit = lunar_lite::four_pillars_from_solar_date_with_options(
        date,
        time_idx,

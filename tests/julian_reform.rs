...
use lunar_lite::{
    MonthDivide, SolarDate, StemBranchOptions, YearDivide,
    get_heavenly_stem_and_earthly_branch_by_solar_date,
    get_heavenly_stem_and_earthly_branch_by_solar_date_with_options, solar_to_lunar,
};
...
fn four_pillar_exact_does_not_collapse_julian_leap_day() {
    ...
    let feb28 = get_heavenly_stem_and_earthly_branch_by_solar_date_with_options(
        solar(1500, 2, 28),
        6,
        opts,
    )
    .unwrap();
    let feb29 = get_heavenly_stem_and_earthly_branch_by_solar_date_with_options(
        solar(1500, 2, 29),
        6,
        opts,
    )
    .unwrap();
    let mar01 =
        get_heavenly_stem_and_earthly_branch_by_solar_date_with_options(solar(1500, 3, 1), 6, opts)
            .unwrap();
...
fn julian_leap_day_default_options_compute() {
    ...
    assert!(get_heavenly_stem_and_earthly_branch_by_solar_date(solar(1500, 2, 29), 6).is_ok());
}
